...
const DEFAULT_WORDS = ['CRIMENET', 'SOVEREIGN MESH'];
...
const SplitFlapText = ({
  ...
  useEffect(() => {
    ...
    if ((normalizedPhrases.length <= 1 && !alwaysFlip) || typeof window === 'undefined') {
    ...
    const safeCycleDelay = Math.max(400, Number(cycleDelay) || 7000);
    ...
    const animateTo = targetPhrase => {
      ...
      const plans = targetChars
        .map((targetChar, index) => {
          ...
          if (!alwaysFlip && fromChar === targetChar) return null;
    ...
    const scheduleNext = delay => {
      cycleTimerRef.current = window.setTimeout(() => {
        ...
        if (nextIndex >= normalizedPhrases.length && !loop && !alwaysFlip) return;
  ...
  }, [normalizedPhrases, width, loop, cycleDelay, flipDuration, stagger, flipsPerChar, charset, prefersReducedMotion, alwaysFlip]);
